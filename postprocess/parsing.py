...
if __name__ == '__main__':
    ...
    data = build_plan_format_conversion_prompt(directory=args.output_dir, set_type=args.set_type, model_name=args.model_name, strategy=args.strategy,mode=args.mode, num_samples=args.num_samples)
    output_file = f'{args.tmp_dir}/{args.set_type}_{args.model_name}{suffix}_{args.mode}.txt'

    total_price = 0
    for idx, prompt in enumerate(tqdm(data[:args.num_samples])):
        if prompt == "":
        ...
        results, _, price = prompt_chatgpt("You are a helpful assistant.", index=idx, save_path=output_file,
                                           user_input=prompt, model_name='gpt-3.5-turbo-0125', temperature=0)
        total_price += price
