...
def request_overpass(query, sleep=2.0):
    headers = {
        "User-Agent": "travelplanner-dwell-osm-context/1.0 academic research",
...
def main():
    ...
    parser.add_argument("--limit-pois", type=int, default=None)
    args = parser.parse_args()

    pois = pd.read_csv(args.input)
    ...
    if args.city:
        pois = pois[pois["dataset_city"] == args.city].copy()
    if args.limit_pois is not None:
        pois = pois.head(args.limit_pois).copy()
        print(f"Limited to first {args.limit_pois} POIs for testing.")
    ...
    for city, city_pois in pois.groupby("dataset_city"):
        print(f"Processing city: {city}, POIs: {len(city_pois)}")

        tiles = build_tiles(city_pois, tile_size_deg=args.tile_size_deg)

        city_elements = []

        for tile_idx, (min_lat, min_lon, max_lat, max_lon) in enumerate(tiles):
            cache_path = cache_dir / f"osm_context_{city}_tile_{tile_idx}.json"
        ...
        city_out = city_pois.copy().reset_index(drop=True)
        ...
        for feature_class in [
            ...
            summary = summarize_nearby_features(
                city_pois.reset_index(drop=True),
                feature_df,
                feature_class,
            )
            city_out = pd.concat(
                [city_out, summary.reset_index(drop=True)],
                axis=1,
            )

        city_out["parking_availability"] = city_out.apply(
            lambda r: level_from_counts(r, "parking"),
            axis=1,
        )

        city_out["public_transport_availability"] = city_out.apply(
            lambda r: level_from_counts(r, "public_transport"),
            axis=1,
        )

        city_out["osm_safety_proxy"] = city_out.apply(
            lambda r: level_from_counts(r, "osm_safety_proxy"),
            axis=1,
        )

        city_out["osm_security_proxy"] = city_out.apply(
            lambda r: level_from_counts(r, "osm_security_proxy"),
            axis=1,
        )

        all_city_outputs.append(city_out)

    final = pd.concat(all_city_outputs, ignore_index=True)
    ...
    for col in [
        ...
        print(f"\n{col}:")
        print(final[col].value_counts(dropna=False))
